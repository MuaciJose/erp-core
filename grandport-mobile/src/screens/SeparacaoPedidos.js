...
export default function SeparacaoPedidos({ onVoltar }) {
    ...
    // 🚀 BLINDAGEM MATEMÁTICA: Impede divisão por zero e quebra de layout
    let progresso = 0;
    if (pedidoAtual && pedidoAtual.itens && pedidoAtual.itens.length > 0) {
        const totalSolicitado = pedidoAtual.itens.reduce((acc, i) => acc + i.qtdeSolicitada, 0);
        const totalSeparado = pedidoAtual.itens.reduce((acc, i) => acc + i.qtdeSeparada, 0);
        if (totalSolicitado > 0) {
            progresso = (totalSeparado / totalSolicitado) * 100;
        }
    }
...
const styles = StyleSheet.create({
    rootContainer: { flex: 1, backgroundColor: '#f8fafc' }, // 🚀 A COR DE FUNDO SALVADORA!
    container: { flex: 1 },

    header: { flexDirection: 'row', alignItems: 'center', padding: 20, paddingTop: Platform.OS === 'ios' ? 40 : 50, backgroundColor: '#fff', borderBottomWidth: 1, borderColor: '#e2e8f0' },
