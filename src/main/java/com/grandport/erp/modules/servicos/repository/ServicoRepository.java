...
public interface ServicoRepository extends JpaRepository<Servico, Long> {
    ...
    @Query("SELECT s FROM Servico s WHERE LOWER(s.nome) LIKE LOWER(CONCAT('%', :busca, '%')) OR LOWER(s.codigo) LIKE LOWER(CONCAT('%', :busca, '%'))")
    List<Servico> buscarPorNomeOuCodigo(String busca);
}