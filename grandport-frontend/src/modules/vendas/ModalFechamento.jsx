...
export const ModalFechamento = ({ isOpen, onClose, onFinalizar, totalVenda, clienteSelecionado }) => {
    ...
    const MetodoPagamento = ({ id, icon, label, hotkey }) => (
        <button 
        ...
        <div className="fixed inset-0 bg-black bg-opacity-60 flex justify-center items-center z-50">
            <div className="bg-white rounded-lg shadow-2xl p-8 w-full max-w-3xl transform transition-all">
                ...
                <div className="grid grid-cols-2 gap-4 mb-6">
                    <MetodoPagamento id="DINHEIRO" icon={<DollarSign className="text-green-600" />} label="Dinheiro" hotkey="F5" />
                    <MetodoPagamento id="CARTAO" icon={<CreditCard className="text-blue-600" />} label="Cartão" hotkey="F6" />
                    <MetodoPagamento id="PIX" icon={<Smartphone className="text-purple-600" />} label="PIX" hotkey="F7" />
                    <MetodoPagamento id="A_PRAZO" icon={<UserCheck className="text-orange-600" />} label="A Prazo / Fiado" hotkey="F8" />
                </div>
                    ...
                    <div className="p-4 bg-yellow-50 rounded-lg border border-yellow-300 mt-4">
                        ...
                        <div className="flex gap-4 items-end mt-2">
                            ...
                            <div className="flex-1">
                                ...
                                <p className={`text-4xl font-black ${troco > 0 ? 'text-green-600' : 'text-gray-500'}`}>{formatCurrency(troco)}</p>
                            </div>
                        </div>
                    </div>
                )}
                ...
                {metodo === 'A_PRAZO' && (
                    <div className={`p-4 rounded-lg border ${clienteSelecionado ? 'bg-green-50 border-green-200' : 'bg-red-50 border-red-200'}`}>
                        {clienteSelecionado ? (
                            <div>
