...
export const OrcamentoPedido = ({ orcamentoParaEditar, onVoltar }) => {
    ...
    const acionarWhatsApp = async () => {
        ...
        try {
            ...
            if (e.response?.data) mensagemTraduzida = e.response.data.message || e.response.data;
            toast.error(`Falha no envio: ${mensagemTraduzida}`, { id: loadId, duration: 5000 });
        }
    };
            ...
            <div className="p-8 max-w-7xl mx-auto flex flex-col h-full animate-fade-in relative print:hidden">
                ...
                {onVoltar && (
                    <div className="mb-4">
                        <button
                            onClick={onVoltar}
                            title="Voltar para a listagem principal"
                    ...
                    <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 mb-6 border-b border-slate-100 pb-4">
                        ...
                        <div className="flex flex-wrap items-center gap-2">
                            ...
                            <div className="flex items-center rounded-lg border border-green-200 bg-green-50 overflow-hidden ml-1">
                                ...
                                <button onClick={acionarWhatsApp} title="Enviar PDF deste documento para o WhatsApp do cliente" className="px-3 py-2 text-green-700 hover:bg-green-100 text-xs font-black flex items-center gap-2 transition-colors">
                ...
                <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-sm flex items-center justify-center z-[200] p-4 print:hidden">
                    <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md overflow-hidden animate-fade-in">
                        <div className="bg-red-600 p-6 flex justify-between items-center text-white">
                            ...
                            <button onClick={() => setModalVendaPerdidaAberto(false)} title="Fechar e cancelar registro de perda" className="hover:text-red-200 transition-colors"><X size={24}/></button>
                        </div>
                        <div className="p-6 space-y-5 bg-slate-50">
                            ...
                            <div>
                                ...
                                <select value={motivoVendaPerdida} onChange={(e) => setMotivoVendaPerdida(e.target.value)} className="w-full p-3 border-2 border-slate-200 rounded-xl font-bold text-slate-700 focus:border-red-500 outline-none mt-1 bg-white shadow-sm">
                            ...
                            <button onClick={registrarVendaPerdida} title="Salvar registro de venda não concretizada" className="w-full bg-red-600 hover:bg-red-700 text-white font-black py-4 rounded-xl shadow-lg mt-2 transition-colors flex items-center justify-center gap-2">
                    ...
                    <table className="w-full mb-6 border-collapse">
                        ...
                        {itens.map((item, index) => (
                            <tr key={index} className="border-b border-slate-50">
                                <td className="p-1 text-[8.5px] font-mono">{item.codigo}</td><td className="p-1 text-[9px] font-bold uppercase">{item.nome}</td><td className="p-1 text-center text-[9px] font-bold">{item.qtd}</td><td className="p-1 text-right text-[9px]">{item.preco.toFixed(2)}</td><td className="p-1 text-right text-[9px] font-black">{(item.preco * item.qtd).toFixed(2)}</td>
                            </tr>
                        ))}
                        </tbody>
                    </table>
            ...
            {modalListaAberto && (
                <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-sm flex items-center justify-center z-[100] p-4 animate-fade-in print:hidden">
                    <div className="bg-white rounded-3xl shadow-2xl w-full max-w-4xl overflow-hidden flex flex-col max-h-[80vh]">
                        <div className="bg-slate-900 p-6 flex justify-between items-center text-white"><h2 className="text-xl font-black tracking-widest flex items-center gap-2"><FolderOpen /> ORÇAMENTOS PENDENTES</h2><button onClick={() => setModalListaAberto(false)} title="Fechar lista de orçamentos"><X size={24}/></button></div>
                        <div className="overflow-y-auto p-6 bg-slate-50 flex-1">
                            {orcamentosSalvos.map(orc => (
                                <div key={orc.id} onClick={() => carregarOrcamentoLocal(orc)} title={`Reabrir orçamento #${orc.id}`} className="bg-white border border-slate-200 p-5 rounded-2xl mb-4 flex justify-between items-center cursor-pointer hover:shadow-md transition-shadow">
