...
export const OrcamentoPedido = () => {
    ...
    // O SEGREDO DA VELOCIDADE: Navegação por Teclado
    ...
    const enviarParaCaixa = async () => {
        ...
        try {
            const res = await api.post('/api/vendas/pedido', payloadPedido);
            alert(`Sucesso! O Pedido #${res.data.id} foi enviado para a fila da Caixa.`);
            limparEcra();
        } catch (error) {
            console.error("Erro ao enviar pedido para o caixa:", error);
            alert("Erro ao enviar pedido para o caixa: " + (error.response?.data?.message || error.message));
        }
    };

    const handleImprimir = () => {
        if (itens.length === 0) {
            return alert("Adicione peças ao orçamento antes de imprimir.");
        }
        window.print();
    };
    ...
    return (
        <>
            ...
            <div className="p-8 max-w-7xl mx-auto flex flex-col h-[90vh] animate-fade-in relative print:hidden">
                ...
                <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-200 mb-6 flex flex-col md:flex-row justify-between items-start md:items-center gap-6 z-40">
                    <div className="flex-1 w-full">
                        ...
                        {/* BLOCO DE BUSCA DE CLIENTE E VEÍCULO */}
                        ...
                        {/* NOVO: PAINEL DE DETALHES COMPLETOS DO CLIENTE E CARRO */}
                        {clienteSelecionado && (
                                            ...
                                            <div className="text-right bg-white p-2 rounded-lg border border-slate-200 shadow-sm">
                                                ...
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    <input 
                                                        ...
                                                        title="Atualize a KM se o cliente informar um valor novo"
                                                    />
                ...
                {/* RODAPÉ FINANCEIRO E BOTÕES DE AÇÃO */}
                <div className="bg-slate-900 text-white rounded-b-3xl border-t-4 border-blue-500 shadow-2xl flex flex-col md:flex-row justify-between p-6 z-20 relative">
                    <div className="flex flex-col justify-end gap-3 mb-4 md:mb-0">
                        <button onClick={salvarOrcamento} className="px-6 py-3 bg-slate-800 hover:bg-slate-700 font-bold rounded-xl flex items-center justify-center gap-2 transition-colors border border-slate-700">
                            <Save size={18} /> GUARDAR PARA DEPOIS
                        </button>
                        {modo === 'ORCAMENTO' ? (
                            ...
                            <button onClick={enviarParaCaixa} disabled={itens.length === 0} className="px-6 py-4 bg-green-500 hover:bg-green-600 font-black text-slate-900 rounded-xl flex items-center justify-center gap-2 shadow-lg disabled:opacity-50 transition-all">
                                <CheckCircle size={20} /> ENVIAR PARA A CAIXA
                            </button>
                        )}
                    </div>
            ...
            {/* ========================================================= */}
            {/* LAYOUT DE IMPRESSÃO (A4) - MOSTRANDO CARRO E KM           */}
            {/* ========================================================= */}
            <div className="hidden print:block bg-white text-black p-8 font-sans w-full max-w-4xl mx-auto">
                ...
                {/* NOVO: DADOS DO CLIENTE E CARRO DETALHADOS NA IMPRESSÃO */}
                <div className="border border-black p-4 mb-6 rounded-lg flex justify-between">
                    ...
                    {veiculoDetalhado && (
                        <div className="text-right">
                            ...
                            <p className="text-sm font-black mt-1">KM Atual: {veiculoDetalhado.km} km</p>
                        </div>
                    )}
                </div>
                ...
                <div className="flex justify-end mb-12">
                    <div className="w-64">
                        ...
                        <div className="flex justify-between border-b border-gray-300 py-1">
                            ...
                            <span className="text-sm">- R$ {valorDescontoReal.toFixed(2)}</span>
                        </div>
                        <div className="flex justify-between mt-2 pt-2 border-t-2 border-black">
                            ...
                            <span className="text-2xl font-black">R$ {totalFinal.toFixed(2)}</span>
                        </div>
                    </div>
                </div>
                ...
                <div className="text-center mt-12 pt-8 border-t border-gray-300">
                    {modo === 'ORCAMENTO' && (
                        <p className="text-xs font-bold uppercase mb-8">** Este orçamento é válido por 5 dias. **</p>
                    )}
                            ...
                            {orcamentosSalvos.length === 0 ? (
                                ...
                                <div className="grid gap-4">
                                    {orcamentosSalvos.map(orc => (
                                        <div key={orc.id} onClick={() => carregarOrcamento(orc)} className="bg-white border border-slate-200 p-5 rounded-2xl shadow-sm flex flex-col md:flex-row justify-between items-center hover:shadow-md transition-shadow cursor-pointer">
                                            <div>
                                            ...
                                            <div className="flex items-center gap-6 mt-4 md:mt-0">
                                                <div className="text-right">
                                                    ...
                                                    <p className="font-black text-xl text-slate-800">R$ {orc.valor.toFixed(2)}</p>
                                                </div>
